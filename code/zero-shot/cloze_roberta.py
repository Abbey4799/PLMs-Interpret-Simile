...
def cloze_test(data):
    ...
    for ii in tqdm(range(len(text_list))):
        ...
        indexed_tokens = tokenizer(text,add_prefix_space = True)['input_ids']
        ...
        candidates_ids = tokenizer(cvt_candidates, add_special_tokens=False, add_prefix_space = True)['input_ids']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_data_path', default='roberta-large', type=str)
    ...
    tokenizer = RobertaTokenizer.from_pretrained(arg.model_data_path)
    language_model = RobertaForMaskedLM.from_pretrained(arg.model_data_path)
